...
public class TaskActivity extends BindViewActivity<ActivityTaskBinding>{
    ...
    public void startTask() {
        publishMoments("小红", "小花", "小华", "小华", "小华", "小华", "小华", "小华", "小华", "小华", "小华", "小华");
    }
                    ...
                    protected String execute(Request param) throws Throwable {
                        param.setListener(new SimpleListener<String>() {
                            ...
                            public void onResponseSuccess(Request request, String result) {
                                if (result == null) {
                                    stopTask();
                                    FastLog.d("next onResponseSuccess() but data is null");
                                }else {
                                    FastLog.d(" next onResponseSuccess()  thread=" + Thread.currentThread().getName());
                                }
                            }

                            @Override
                            public void onError(Request request, Exception error) {
                                FastLog.d(" next onError()  thread=" + Thread.currentThread().getName() +error);
                            }
                        });
        ...
        startTask(Task.begin(images)
                ...
                .next(new Action<String,String>() {
                    ...
                    protected String execute(String param) throws Throwable {
                        FastLog.d(" next " + Thread.currentThread().getName()+" <+param");
                        return "<"+param;
                    }
                })
                .next(new Action<String,String>() {
                ...
                .filter(new Action<String, Boolean>() {
                    @Override
                    protected Boolean execute(String param) throws Throwable {
